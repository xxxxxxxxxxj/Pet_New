    ...
    protected void convert(final BaseViewHolder helper, CollectChargeBean.DataBean item) {
        ...
        if (item != null) {
            ...
            if(StringUtil.isNotEmpty(item.getPrice())){
                if(item.getPrice().endsWith("元/度")){
                    StringUtil.setText(tv_item_collectcharge_cdf, "充电费：" + item.getPrice(), "", View.VISIBLE, View.VISIBLE);
                }else{
                    StringUtil.setText(tv_item_collectcharge_cdf, "充电费：" + item.getPrice() + "元/度", "", View.VISIBLE, View.VISIBLE);
                }
            }else{
                StringUtil.setText(tv_item_collectcharge_cdf, "充电费：" + item.getPrice() + "元/度", "", View.VISIBLE, View.VISIBLE);
            }
            if(StringUtil.isNotEmpty(item.getServiceFee())){
                if(item.getServiceFee().endsWith("元/度")){
