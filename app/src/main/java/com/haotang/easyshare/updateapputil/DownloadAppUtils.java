        ...
        FileDownloader.getImpl().create(localUrl)
                ...
                .setListener(new FileDownloadLargeFileListener() {
                    ...
                    protected void progress(BaseDownloadTask task, long soFarBytes, long totalBytes) {
                        if (localisUpgrade == 1) {
                            DevRing.busManager().postEvent(new UpdateAppEvent(soFarBytes, totalBytes, UpdateAppEvent.DOWNLOADING));
                        } else {
                    ...
                    protected void completed(BaseDownloadTask task) {
                        if (localisUpgrade == 1) {
                            DevRing.busManager().postEvent(new UpdateAppEvent(UpdateAppEvent.DOWNLOAD_COMPLETE));
                        } else {
                    ...
                    protected void error(BaseDownloadTask task, Throwable e) {
                        if (localisUpgrade == 1) {
                            DevRing.busManager().postEvent(new UpdateAppEvent(UpdateAppEvent.DOWNLOAD_FAIL));
                        } else {
