package com.haotang.easyshare.verticalbanner;
...
public class VerticalBannerView extends LinearLayout implements
		BaseBannerAdapter.OnDataChangedListener {

	private float mBannerHeight = TypedValue
	...
	public void start() {
		if (mAdapter == null) {
			throw new RuntimeException(
					"you must call setAdapter() before start");
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isInEditMode()) {
			...
			mDebugPaint.setTextSize(TypedValue.applyDimension(
					TypedValue.COMPLEX_UNIT_SP, 16, getResources()
							.getDisplayMetrics()));
			mDebugPaint.setStyle(Paint.Style.STROKE);
			canvas.drawText("banner is here", 20, getHeight() * 2 / 3,
					mDebugPaint);
		}
	}
	...
	private void performSwitch() {
		ObjectAnimator animator1 = ObjectAnimator.ofFloat(mFirstView,
				"translationY", mFirstView.getTranslationY() - mBannerHeight);
		ObjectAnimator animator2 = ObjectAnimator.ofFloat(mSecondView,
				"translationY", mSecondView.getTranslationY() - mBannerHeight);
			...
			public void onAnimationEnd(Animator animation) {
				try {
					...
					if (onChangeListener != null) {
						if (mAdapter.getCount() > 0) {
							onChangeListener.OnChange(mPosition
									% mAdapter.getCount());
						}
					}
				} catch (Exception e) {
